...
import NewAnswer from './new-answer/NewAnswer';
    ...
    return (
        <div className={styles.question_input}>
            <div className={styles.question_input_content}>
                ...
                />
                <div className={styles.question_input_content_multi}>
                    <p>Má tato otázka více správných odpovědí?</p>
                    <input
                        type='checkbox'
                ...
                <div className={styles.question_input_content_answers}>
                    <p>Odpovědi</p>
                    <div className={styles.question_input_content_answers_list}>
                        {question.options.map((answer, answerIndex) => (
                            <AnswerCard
                                key={answerIndex}
                                answer={answer}
                                onChange={(e) => onAnswerChange(answerIndex, e.target.value)}
                                active={isCorrect(answerIndex)}
                                onSetActive={() => onSetCorrectAnswer(answerIndex)}
                                onDelete={() => onDeleteAnswer(answerIndex)}
                            />
                        ))}
                        <NewAnswer onClick={onAddAnswer} />
                    </div>
                </div>
            </div>
            <button
                type="button"
                onClick={onDelete}
                className={styles.question_input_delete}
            >
