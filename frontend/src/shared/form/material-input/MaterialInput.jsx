...
export function UrlInput({ onChange }) {
    ...
    return (
        <div className={styles.material_input_group}>
...
export function FileInput({ onChange }) {
    ...
    return (
        <div className={styles.material_input_group}>
            <div className={styles.file_input_row}>
                <input
                    type="file"
            ...
            <div className={styles.material_input}>
                <input
                    placeholder='Název (nepovinné — auto-doplní se)'
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                />
            </div>
            <div className={styles.material_input}>
                ...
                <button
                    type="button"
                    onClick={(e) => {
                        const fileInput = e.currentTarget.closest(`.${styles.material_input_group}`).querySelector('input[type="file"]');
                        const selectedFile = fileInput?.files[0];
                        if (!selectedFile) return;

                        if (onChange) {
                            onChange({
                                name: name.trim() || selectedFile.name.replace(/\.[^/.]+$/, ''),
                                description: description.trim(),
                                file: selectedFile,
