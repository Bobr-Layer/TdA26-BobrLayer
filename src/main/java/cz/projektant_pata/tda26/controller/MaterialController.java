...
public class MaterialController {
        ...
        public ResponseEntity<MaterialResponseDTO> update(
                        ...
                        @RequestParam(value = "description", required = false) String description) {
                Material updated = service.update(moduleUuid, materialUuid, file, name, description); // ✅ bylo:
                                                                                                      // courseUuid
                return ResponseEntity.ok(mapper.toResponse(updated));
        }
        ...
        @PostMapping("/{materialUuid}/track")
        public ResponseEntity<Void> track(
                        @PathVariable UUID courseUuid,
                        @PathVariable UUID moduleUuid,
                        @PathVariable UUID materialUuid) {
                service.trackAccess(courseUuid, moduleUuid, materialUuid);
                return ResponseEntity.noContent().build();
        }
}
